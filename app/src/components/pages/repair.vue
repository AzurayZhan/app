<template>
  <div class="box">
    <div class="header">
      <van-nav-bar title="商家列表" @click-left="toBack()">
    ...
    <div class="main">
      <div class="list">
        <router-link :to="'/repairDetail?id='+item.id" v-for="item in repair" :key="item.id">
          <div class="des">
            <h2>
              <span>{{item.name}}</span>
...
export default {
  ...
  mounted() {
    ajax(
      ...
      d => {
        console.log(d);
        this.repair = d.data.data;
        this.repair.map(item => {
